...
<p style="text-align:justify;">From the result of the model, DUI1802, BKGRTOABC,BAPTISTSP, ENTRECP, and URBANP (significant for significance level 0.1) variables are seen as significant variables. This shows that  alcohol sales has no effect on the domestic violance. Moreover, The higher education, weighted average distance to nearest bar selling liquor, income per capita, and percentage of unemployed people do not have a significant effect on liquor sales. R-squared is 0.4572, which is quite okay.</p>
...
Moran’s I statistic is significant (p < 0.05) Moran's Statistic is ~0.02 which means that there is a significant evidence that negative spatial correlation exists between the residuals and we should use a spatial model. 
...
P-value is 0.03043 so we reject H0 homoscedasticity which means we have heteroscedasticity.
...
We can see from the results that both negative and positive residuals have positive spatial correlation. Which another indicator that we should continue with spatial model instead of OLS.
...
DUI1802, ENTRECP significant while the rest are insignificant and if it comes to thetas all of them are insignificant. We can also see from the result that Rho and Lambda are insignificant as well and model's AIC is 1156.2, which we are seeking for the smallest AIC for the best model. Since AIC for lm is bigger than AIC we should continue to search for spatial model.
