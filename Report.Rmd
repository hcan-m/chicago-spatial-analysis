...
Before jumping construction of models, first we need a spatial weight matrix for spatial models in the following part of the model. 

### Preparing Spatial Weights Matrix

Structure of the neighborhood is key factor while constructing a spatial model. In our analysis, we tried to create few neighborhood matrix which are contiguity matrix and k nearest neighbors (knn) matrix. We also checked neighbours for symmetry. 
...
Secondly, we created spatial weights matrix with k nearest neighbours. The number of closest neighbours is selected 3. The result map is provided below:
...
We can check whether the negihbourhood matrix is symmetric. The symmetry test shows that, the matrix is not symmetric since the test result gives `r print(is.symmetric.nb(spa.knn.nb))`. We can turn it to syymetric matrix as below:
...
We can also save symmetric matrix as listw class. The results below shows that average number of links is nearly 3.6.
...
Lastly, we can calculate at what distance all units have at least one neighbour. At 0.7975231 km, all units have at least one neighbour. 
...
A measure of global spatial autocorrelation: Moran's I 

```{r echo=FALSE}
moranTest <- moran.test(spadata$SALESPC, cont.listw)
moranTest
```
...
```{r}
ols <- lm(SALESPC ~ DUI1802 + SOMECOLLP + BKGRTOABC + BAPTISTSP + BKGRTOMIX + ENTRECP + UNEMPP + PCI + URBANP + FVPTHH02, data = spadata)
summary(ols)
```
<<<<<<< HEAD
...
=======
We can see from our model BKGRTOABC   ENTRECP      UNEMPP       significant and DUI1802 could be in 0.1 interval
 R-squared:  0.4572 which is quite okay.
>>>>>>> d0f58b8d0c8ad4fd87d5fe1019f55bb9edd8f0e5
...
```{r}
lmMoranTest <- lm.morantest(ols,cont.listw) # are residuals spatially random?
lmMoranTest

```

p-value is very small so we have to reject null hypothesis and Moran's Statistic is  -0.005 which means that there is a significant evidence that negative spatial correlation exists between the residuals and we should use a spatial model.
...
We can see on the Moran scatterplot how our dependent variable's spatial correlation  

We can also cluster negatives and positives next to each other and check how they correlate. In order to do that we took the residuals and cluster them and after that we used join count test.
